...
def collect_image_paths(data_dir):
    ...
    path_generator = MyFilePathGenerator()
    ...
    path_to_root = path_generator.get_full_file_path(id_=,
                                                     root_dir=data_dir,
                                                     file_key=image_keys,
                                                     file_extension='.mha')

    crawler_ = futil.FileSystemDataCrawler(path_to_root,
                                           image_keys,
                                           MyFilePathGenerator(),
                                           dir_filter,
                                           '.mha')
...
def load_images(image_paths):
    ...
    float_img = sitk.ReadImage(fileName=image_paths, outputPixelType=sitk.sitkFloat32)
    uint_img = sitk.ReadImage(fileName=image_paths, outputPixelType=sitk.sitkUInt8)
    image_dict = {
        structure.BrainImageTypes.T1w: float_img,  # todo: modify here
        structure.BrainImageTypes.GroundTruth: uint_img # todo: modify here
    }
...
def register_images(image_dict, atlas_img):
    ...
    registered_t1 = sitk.ImageRegistrationMethod(registration, registration_params)  # todo: modify here
...
def preprocess_filter_rescale_t1(image_dict, new_min_val, new_max_val):
    ...
    image_T1 = image_dict[structure.BrainImageTypes.T1w]
    filter_ = MinMaxRescaleFilter()  # todo: modify here
    filter_params = MinMaxRescaleFilterParams(new_min_val, new_max_val)  # todo: modify here
    minmax_rescaled_img = filter_.execute(img=image_T1, params=filter_params)    # todo: modify here
...
def extract_feature_median_t1(image_dict):
    ...
    median_img = filter_.execute(img=image_T1) # todo: modify here
