...
def load_image(img_path, is_label_img):
    ...
    img = sitk.ReadImage(img_path, outputPixelType=pixel_type, imageIO="PNGImageIO")
...
def register_images(img, label_img, atlas_img):
    ...
    transform = sitk.ImageRegistrationMethod(fixed=atlas_img, moving=img)
    ...
    registered_img = sitk.Resample(img,referenceImage=atlas_img, transform=transform, interpolator=sitkLinear,default_value=0.0, outputPixelType=img.GetPixelIDValue() )
    ...
    registered_label =sitk.Resample(label_img, referenceImage=atlas_img, transform=transform,interpolator=sitkNearestNeighbor, default_value=0.0, outputPixelType=label_img.GetPixelIDValue() )
...
def preprocess_rescale_numpy(np_img, new_min_val, new_max_val):
    ...
    rescaled_np_img = np.resize(np_img, (new_min_val, new_max_val))
